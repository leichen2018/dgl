...
def pm_pd(g):
    ...
    deg_src_inverse = g.in_degrees(g.all_edges()[0]).float() - 1
    deg_src_inverse = th.where(deg_src_inverse>0, 1/deg_src_inverse, th.zeros(deg_src_inverse.size()))
    ...
    pm = th.sparse.FloatTensor(pmpd_src_i, deg_src_inverse, th.Size([g.number_of_nodes(), g.number_of_edges()])).to_dense()
...
class GNNModule(nn.Module):
    ...
    def forward(self, x0, x1, y, deg_g, g_a1, lg_a1, pm, pd):
        ...
        x_r = self.theta_a_r(th.mm(g_a1, x1)) + self.theta_deg_r(th.mm(deg_g, x0)) + self.theta_y_r(pd_y)
        x = self.theta_a(th.mm(g_a1, x1)) + self.theta_deg(th.mm(deg_g, x0)) + self.theta_y(pd_y)
...
class GNN(nn.Module):
    ...
    def forward(self, deg_g, deg_lg, g_a1, g_d, lg_a1, pm, pd):
        x0, y = deg_g, deg_lg
        ...
        for module in self.module_list:
            x0, x1, y = module(x0, x1, y, g_d, g_a1, lg_a1, pm, pd)
        
        x = self.module_final(x0, x1, y, g_d, g_a1, pm, pd)
        return x

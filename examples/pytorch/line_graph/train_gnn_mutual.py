...
model = gnn_mutual.GNN(args.n_layers, args.n_communities)
model.to(dev)
...
if args.model_path != 'None':
    with th.no_grad():
        ...
        print('Model load successfully from ' + args.model_path + '.pkl !')
        '''
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        with th.no_grad():	
            ...
            g_adj = g_adj.to_dense().to(dev)
	
            s_buildgraph += time.time() - t_bg

        loss, overlap, t_forward, t_backward = step(deg_g, g_adj)

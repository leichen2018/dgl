...
class GNNModule(nn.Module):
    def __init__(self, in_feats, out_feats, radius, dev):
        ...
        self.theta_a, self.theta_a2, self.theta_deg, self.theta_y = new_linear(), new_linear(), new_linear(), new_linear()

        self.gamma_a, self.gamma_x = new_linear(), new_linear_()

        ## Start relu part
        self.theta_a_r, self.theta_a2_r, self.theta_deg_r, self.theta_y_r = new_linear(), new_linear(), new_linear(), new_linear()
    ...
    def forward(self, x0, x1, y, deg_g, g_a1, lg_a1, pm, pd):
        ...
        x_r = self.theta_a_r(th.mm(g_a1, x1)) + self.theta_a2_r(th.mm(g_a1, th.mm(g_a1, x0))) + self.theta_deg_r((deg_g - 1) * x0) + self.theta_y_r(pd_y)
        x = self.theta_a(th.mm(g_a1, x1)) + self.theta_a2(th.mm(g_a1, th.mm(g_a1, x0))) + self.theta_deg((deg_g - 1) * x0) + self.theta_y(pd_y)

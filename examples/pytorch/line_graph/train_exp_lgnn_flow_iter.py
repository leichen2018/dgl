...
import sbm
import exp_lgnn_flow_iter
from exp_lgnn_flow_iter import pm_pd
...
model = exp_lgnn_flow_iter.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, deg_g, deg_lg, g_a1, g_d, lg_a1, pm, pd):
    ...
    t0 = time.time()
    z = model(deg_g, deg_lg, g_a1, g_d, lg_a1, pm, pd)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        ...
        loss, overlap, t_forward, t_backward = step(i, j, deg_g, deg_lg, Du_DD1_A, A_DD1, adj_diag_lg_deg_inverse, pm, pd)

...
import sbm
import exp_gnn_iter
from exp_gnn_iter import pm_pd
...
model = exp_gnn_iter.GNN(feats, args.radius, K, dev).to(dev)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        ...
        loss, overlap, t_forward, t_backward = step(i, j, deg_g, g_a1)

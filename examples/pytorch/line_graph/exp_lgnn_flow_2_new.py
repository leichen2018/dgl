...
class GNNModule(nn.Module):
    def __init__(self, in_feats, out_feats, radius, dev):
        ...
        self.theta_x, self.theta_ad, self.theta_ad_2, self.theta_dad, self.theta_y = \
            new_linear(), new_linear(), new_linear(), new_linear(), new_linear()

        self.gamma_y, self.gamma_ad, self.gamma_ad_2, self.gamma_x = \
            new_linear(), new_linear(), new_linear(), new_linear_()

        ## Start relu part
        self.theta_x_r, self.theta_ad_r, self.theta_ad_r_2, self.theta_dad_r, self.theta_y_r = \
            new_linear(), new_linear(), new_linear(), new_linear(), new_linear()

        self.gamma_y_r, self.gamma_ad_r, self.gamma_ad_r_2, self.gamma_x_r = \
            new_linear(), new_linear(), new_linear(), new_linear_()
        ## End relu part
    ...
    def forward(self, x, y, g_a_d, g_d_a_d, lg_a_d, pm, pd):
        ...
        x_r = self.theta_x_r(x) + self.theta_ad_r(th.mm(g_a_d, x)) + self.theta_ad_r_2(th.mm(g_a_d, th.mm(g_a_d, x))) + self.theta_dad_r(th.mm(g_d_a_d, x)) + self.theta_y_r(pd_y)
        x = self.theta_x(x) + self.theta_ad(th.mm(g_a_d, x)) + self.theta_ad_2(th.mm(g_a_d, th.mm(g_a_d, x))) + self.theta_dad(th.mm(g_d_a_d, x)) + self.theta_y(pd_y)
        x = th.cat([x, F.relu(x_r)], 1)
        x = self.bn_x(x.unsqueeze(2).unsqueeze(3)).squeeze(3).squeeze(2)

        pm_x = th.mm(pm.t(), x)

        y_r = self.gamma_y_r(y) + self.gamma_ad_r(th.mm(lg_a_d, y)) + self.gamma_ad_r_2(th.mm(lg_a_d, th.mm(lg_a_d, y))) + self.gamma_x_r(pm_x)
        y = self.gamma_y(y) + self.gamma_ad(th.mm(lg_a_d, y)) + self.gamma_ad_2(th.mm(lg_a_d, th.mm(lg_a_d, y))) + self.gamma_x(pm_x)

...
model = exp_gnn_flow_2.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, g_a_d, g_d_a_d, deg_g):
    """ One step of training. """
    deg_g = deg_g.to(dev)

    t0 = time.time()
    z = model(g_a_d, g_d_a_d, deg_g)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        ...
        loss, overlap, t_forward, t_backward = step(i, j, Du_DD1_A, A_DD1, deg_g)

...
import sbm
import exp_lgnn_nbt_iter
from exp_lgnn_nbt_iter import pm_pd
...
model = exp_lgnn_nbt_iter.GNN(feats, args.radius, K, dev).to(dev)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        ...
        loss, overlap, t_forward, t_backward = step(i, j, deg_g, deg_lg, g_a1, adj_diag_lg_deg_inverse, pm, pd)

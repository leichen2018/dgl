...
class GNNModule(nn.Module):
    ...
    def forward(self, x, g_a_d, g_d_a_d):
        #x_r = self.theta_x_r(x) + self.theta_ad_r(th.mm(g_a_d, x)) + self.theta_ad_2_r(th.mm(g_a_d, th.mm(g_a_d, x))) + self.theta_dad_r(th.mm(g_d_a_d, x))
        x_r = self.theta_ad_r(th.mm(g_a_d, x)) + self.theta_dad_r(th.mm(g_d_a_d, x))
        #x = self.theta_x(x) + self.theta_ad(th.mm(g_a_d, x)) + self.theta_ad_2(th.mm(g_a_d, th.mm(g_a_d, x))) + self.theta_dad(th.mm(g_d_a_d, x))
        x = self.theta_ad(th.mm(g_a_d, x)) + self.theta_dad(th.mm(g_d_a_d, x))
...
class GNNModule_final(nn.Module):
    def __init__(self, in_feats, out_feats, radius, dev):
        ...
        self.theta_x, self.theta_ad, self.theta_dad = \

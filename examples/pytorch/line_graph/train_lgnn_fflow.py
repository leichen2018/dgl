...
if args.model == 'lgnn':
    model = lgnn_fflow.GNN(feats, args.radius, K, dev).to(dev)
elif args.model == 'lgnn_relu':
    model = lgnn_relu.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, g_a_d, g_d_a_d, lg_a_d, pm, pd, deg_g, deg_lg):
    ...
    t0 = time.time()
    z = model(g_a_d, g_d_a_d, lg_a_d, pm, pd, deg_g, deg_lg)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        ...
        loss, overlap, t_forward, t_backward = step(i, j, adj_diag_deg_inverse, diag_deg_inverse_adj_deg_inverse, adj_diag_lg_deg_inverse, pm, pd, deg_g, deg_lg)

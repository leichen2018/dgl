...
import sbm
import exp_lgnn_flow_2_new
from exp_lgnn_flow_2_new import pm_pd
...
model = exp_lgnn_flow_2_new.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, g_a_d, g_d_a_d, lg_a1, deg_g, deg_lg, pm, pd):
    ...
    t0 = time.time()
    z = model(g_a_d, g_d_a_d, lg_a1, pm, pd, deg_g, deg_lg)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        with th.no_grad():	
            ...
            '''
            deg_inverse = th.where(deg_g>1, 1/(deg_g-1), th.zeros(deg_g.size()).to('cuda:0'))
            diag_deg_inverse = th.diag(deg_inverse.squeeze(1)).to('cuda:0')
            adj_diag_deg_inverse = th.mm(g_a1, diag_deg_inverse)
            adj_deg_inverse = th.mm(g_a1, deg_inverse)
            diag_deg_inverse_adj_deg_inverse = th.diag((deg_inverse * adj_deg_inverse).squeeze(1)).to('cuda:0')
            '''

            DD1 = th.mm(deg_g-1, deg_g.squeeze(1).unsqueeze(0)-1)-1
            DD1 = th.where(DD1>0, 1/DD1, th.zeros(DD1.size()).to('cuda:0'))
            Du = th.diag(deg_g.squeeze(1)-1).to('cuda:0')
            Du_DD1 = th.mm(Du, DD1)
            Du_DD1_A = Du_DD1 * g_a1
            A_DD1 = th.diag(th.sum(g_a1 * DD1, dim = 1)).to('cuda:0')

            lg_a1 = lg_adj.to_dense()
            lg_deg_inverse = th.where(deg_lg>0, 1/deg_lg, th.zeros(deg_lg.size()).to('cuda:0'))
            diag_lg_deg_inverse = th.diag(lg_deg_inverse.squeeze(1)).to('cuda:0')
            adj_diag_lg_deg_inverse = th.mm(lg_a1, diag_lg_deg_inverse) - 1/lg.number_of_nodes()

            pm, pd = pm_pd(g)
	
            s_buildgraph += time.time() - t_bg

        #loss, overlap, t_forward, t_backward = step(i, j, adj_diag_deg_inverse, diag_deg_inverse_adj_deg_inverse, deg_g)
        loss, overlap, t_forward, t_backward = step(i, j, Du_DD1_A, A_DD1, adj_diag_lg_deg_inverse, deg_g, deg_lg, pm, pd)

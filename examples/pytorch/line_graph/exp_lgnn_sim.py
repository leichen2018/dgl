...
class GNNModule(nn.Module):
    def __init__(self, in_feats, out_feats, radius, dev):
        ...
        self.gamma_a, self.gamma_x, self.gamma_i, self.gamma_a_2 = new_linear(), new_linear_(), new_linear(), new_linear()

        ## Start relu part
        self.theta_y_r = new_linear()

        self.gamma_a_r, self.gamma_x_r, self.gamma_i_r, self.gamma_a_2_r = new_linear(), new_linear_(), new_linear(), new_linear()
        ## End relu part
    ...
    def forward(self, y, deg_g, g_a1, lg_a1, pm, pd):
        ...
        #y_r = self.gamma_i_r(y) + self.gamma_a_r(th.mm(lg_a1, y)) + self.gamma_a_2_r(th.mm(lg_a1, th.mm(lg_a1, y))) #+ self.gamma_x_r(pm_x)
        #y = self.gamma_i(y) + self.gamma_a(th.mm(lg_a1, y)) + self.gamma_a_2(th.mm(lg_a1, th.mm(lg_a1, y))) #+ self.gamma_x(pm_x)
...
class MLP(nn.Module):
    def __init__(self, in_feats, hidden_feats, layers):
        super().__init__()
        feats = [in_feats] + [hidden_feats] * layers
        self.modules_linear = nn.ModuleList([nn.Linear(m, n) for m, n in zip(feats[:-1], feats[1:])])
    
    def forward(self, y):
        for layer_linear in self.modules_linear:
            y = layer_linear(y)
            y = F.relu(y)

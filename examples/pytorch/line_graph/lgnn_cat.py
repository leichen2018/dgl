...
class GNNModule(nn.Module):
    ...
    def forward(self, x_list, y_list, deg_g, deg_lg, g_a1, g_a2, lg_a1, lg_a2, pm, pd):
        x = x_list[-1]
        y = y_list[-1]
...
class GNNModule_final(nn.Module):
    def __init__(self, in_feats, out_feats, radius, dev, depth):
        super().__init__()
        self.in_feats = in_feats
        self.out_feats = out_feats
        self.radius = radius
        ...
        new_linear_list = lambda: nn.ModuleList([new_linear() for i in range(radius)])
        new_linear_cat = lambda: nn.ModuleList([new_linear() for i in range(self.depth)])

        new_linear_ = lambda: nn.Linear(out_feats, out_feats)

        self.cat_x = new_linear_cat()

        self.theta_deg, self.theta_y_0, self.theta_y_1 = \
            new_linear(), new_linear(), new_linear()
        self.theta_list = new_linear_list()

    def forward(self, x_cat_list, y_cat_list, deg_g, g_a1, g_a2, pm, pd):
        x = x_cat_list[-1]
        y = y_cat_list[-1]
        ...
        cat_x = sum(theta(z) for theta, z in zip(self.cat_x, x_cat_list[1:]))

        x = cat_x + self.theta_deg(deg_g * x) + sum_x + self.theta_y_0(pm_y) + self.theta_y_1(pd_y)
...
class GNN(nn.Module):
    def __init__(self, feats, radius, n_classes, dev):
        ...
        self.module_final = GNNModule_final(feats[-2], feats[-1], radius, dev, len(feats)-2)

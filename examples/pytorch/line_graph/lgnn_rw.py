...
class GNNModule(nn.Module):
    ...
    def forward(self, x, y, g_w_list, lg_w_list, pm, pd):
        pm_y = th.mm(pm, y)
        pd_y = th.mm(pd, y)        
        ...
        x_r = self.theta_x_r(x) + sum_x_r + self.theta_y_0_r(pm_y) + self.theta_y_1_r(pd_y)
        x = self.theta_x(x) + sum_x + self.theta_y_0(pm_y) + self.theta_y_1(pd_y)
        n = self.out_feats // 2
        ...
        y_r = self.gamma_y_r(y) + sum_y_r + self.gamma_x_0_r(pm_x) + self.gamma_x_1_r(pd_x)
        y = self.gamma_y(y) + sum_y + self.gamma_x_0(pm_x) + self.gamma_x_1(pd_x)
...
class GNNModule_final(nn.Module):
    ...
    def forward(self, x, y, g_w_list, pm, pd):
        pm_y = th.mm(pm, y)
        pd_y = th.mm(pd, y)
...
class GNN(nn.Module):
    ...
    def forward(self, deg_g, deg_lg, g_w_list, lg_w_list, pm, pd):
        x, y = deg_g, deg_lg
        
        for module in self.module_list:
            x, y = module(x, y, g_w_list, lg_w_list, pm, pd)
        
        x = self.module_final(x, y, g_w_list, pm, pd)
        return x

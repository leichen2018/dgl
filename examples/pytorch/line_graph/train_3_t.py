...
model = gnn_3.GNN(feats, args.radius, K, dev).to(dev)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        with th.no_grad():	
            ...
            lg_a1 = lg_adj.to_dense().t()
            lg_a1_a1 = th.sparse.mm(lg_adj, lg_adj.to_dense())
            lg_a2 = th.where(lg_a1_a1>0, th.ones(lg_a1_a1.size()).to('cuda:0'), lg_a1_a1).t()

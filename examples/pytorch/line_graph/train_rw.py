...
model = gnn_rw.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, deg_g, g_w_list):
    """ One step of training. """
    deg_g = deg_g.to(dev)
    t0 = time.time()
    z = model(deg_g, g_w_list)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        with th.no_grad():	
            ...
            for k in range(args.radius):
                g_w = th.mm(th.diag(1/(th.sum(g_w_p, dim = 1) + eps)).to('cuda:0'), g_w_p)

                g_w_list.append(g_w)
                
                g_w_p = th.mm(g_w, g_w.t())
            
        loss, overlap, t_forward, t_backward = step(i, j, deg_g, g_w_list)

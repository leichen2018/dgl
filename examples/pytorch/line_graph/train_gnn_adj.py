...
import sbm
from gnn_adj import aggregate_init, pm_pd
import gnn_adj
import gnn_adj_no2
...
if args.model == 0:
    model = gnn_adj.GNN(feats, args.radius, K, dev).to(dev)
else:
    model = gnn_adj_no2.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, g, deg_g, g_t, g_tt, mask_g_t, mask_g_tt):
    """ One step of training. """
    deg_g = deg_g.to(dev)
    t0 = time.time()
    z = model(g, deg_g, g_t, g_tt, mask_g_t, mask_g_tt)
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        with th.no_grad():	
            t_bg = time.time()
            g, _, deg_g, _, _, _ = sbm.SBM(1, args.n_nodes, K, p, q).__getitem__(0)

            g_t, g_tt, mask_g_t, mask_g_tt = aggregate_init(g)
	
            s_buildgraph += time.time() - t_bg

        loss, overlap, t_forward, t_backward = step(i, j, g, deg_g, g_t, g_tt, mask_g_t, mask_g_tt)

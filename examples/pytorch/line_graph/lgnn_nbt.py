...
class GNNModule(nn.Module):
    ...
    def forward(self, x, y, deg_g, deg_lg, g_a1, g_a2, lg_a1, lg_a2, pm, pd):
        ...
        x_r = self.theta_x_r(x) + self.theta_deg_r(deg_g * x) + sum_x_r + self.theta_y_1_r(pd_y)
        x = self.theta_x(x) + self.theta_deg(deg_g * x) + sum_x + self.theta_y_1(pd_y)
        n = self.out_feats // 2
        ...
        y_r = self.gamma_y_r(y) + self.gamma_deg_r(deg_lg * y) + sum_y_r + self.gamma_x_0_r(pm_x)
        y = self.gamma_y(y) + self.gamma_deg(deg_lg * y) + sum_y + self.gamma_x_0(pm_x)

...
if args.model == 'lgnn':
    model = lgnn_flow.GNN(feats, args.radius, K, dev).to(dev)
...
@from_np
def step(i, j, deg_g, deg_lg, g_a1, g_a2, lg_a1, lg_a2, pm, pd):
...
##    for j, [g, lg, deg_g, deg_lg, pm_pd] in enumerate(training_loader):
    for j in range(args.n_graphs):
        with th.no_grad():	
            ...
            deg_flow = (deg_g[g.edges()[1]] - 1).to('cuda:0')
            deg_flow = th.where(deg_flow>0, 1/deg_flow, th.zeros(deg_flow.size()).to('cuda:0'))

            lg_adj = lg_adj.t().to_dense()
            #lg_a2 = th.mm(lg_adj, lg_adj)
            #lg_a2 = th.where(lg_a2>0, th.ones(lg_a2.size()).to('cuda:0'), lg_a2)
            lg_a1 = th.mm(th.diag(deg_flow.squeeze(1)).to('cuda:0'), lg_adj)
            lg_a2 = th.mm(lg_a1, lg_a1)
            #lg_a2 = th.mm(th.diag(deg_flow.squeeze(1)).to('cuda:0'), lg_a2)
        ...
        loss, overlap, t_forward, t_backward = step(i, j, deg_g, deg_lg, g_a1, g_a2, lg_a1, lg_a2, pm, pd)
